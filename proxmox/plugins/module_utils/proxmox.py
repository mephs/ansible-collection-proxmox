...
def proxmox_auth_argument_spec():
    options = dict(
        api_host=dict(type='str', fallback=(env_fallback, ['PROXMOX_HOST']), required=True, aliases=['api', 'host']),
        api_user=dict(type='str', fallback=(env_fallback, ['PROXMOX_USER']), required=True, aliases=['user']),
        api_password=dict(type='str', fallback=(env_fallback, ['PROXMOX_PASSWORD']), no_log=True),
        api_token_id=dict(type='str', fallback=(env_fallback, ['PROXMOX_TOKEN']), no_log=False, aliases=['token_id', 'token']),
        api_token_secret=dict(type='str', fallback=(env_fallback, ['PROXMOX_SECRET']), no_log=True, aliases=['token_secret', 'secret']),
